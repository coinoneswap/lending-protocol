...
contract ComptrollerG6 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    uint public constant COMP_CLAIM_THRESHOLD = 0.001e18;

    /// @notice The initial COMP index for a market
    uint224 public constant COMP_INITIAL_INDEX = 1e36;

    // closeFactorMantissa must be strictly greater than this value
    uint internal constant CLOSE_FACTOR_MIN_MANTISSA = 0.05e18; // 0.05

    // closeFactorMantissa must not exceed this value
    uint internal constant CLOSE_FACTOR_MAX_MANTISSA = 0.9e18; // 0.9

    // No collateralFactorMantissa may exceed this value
    uint internal constant COLLATERAL_FACTOR_MAX_MANTISSA = 0.9e18; // 0.9
    ...
    function liquidateBorrowAllowed(
        ...
        uint maxClose = mulNeScalarTruncate(Exp({mantissa: closeFactorMantissa}), borrowBalance);
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            vars.sumCollateral = mulNeScalarTruncateAddUInt(vars.tokensToDenom, vars.cTokenBalance, vars.sumCollateral);

            // sumBorrowPlusEffects += oraclePrice * borrowBalance
            vars.sumBorrowPlusEffects = mulNeScalarTruncateAddUInt(vars.oraclePrice, vars.borrowBalance, vars.sumBorrowPlusEffects);

            // Calculate effects of interacting with cTokenModify
            if (asset == cTokenModify) {
                // redeem effect
                // sumBorrowPlusEffects += tokensToDenom * redeemTokens
                vars.sumBorrowPlusEffects = mulNeScalarTruncateAddUInt(vars.tokensToDenom, redeemTokens, vars.sumBorrowPlusEffects);

                // borrow effect
                // sumBorrowPlusEffects += oraclePrice * borrowAmount
                vars.sumBorrowPlusEffects = mulNeScalarTruncateAddUInt(vars.oraclePrice, borrowAmount, vars.sumBorrowPlusEffects);
            }
        }
    ...
    function liquidateCalculateSeizeTokens(address cTokenBorrowed, address cTokenCollateral, uint actualRepayAmount) external view returns (uint, uint) {
        ...
        seizeTokens = mulNeScalarTruncate(ratio, actualRepayAmount);
    ...
    function _supportMarket(CToken cToken) external returns (uint) {
        ...
        require(cToken.IS_CTOKEN(), "This is not a CToken contract!");
    ...
    function _setBorrowCapGuardian(address newBorrowCapGuardian) external {
        ...
        require(borrowCapGuardian != address(0), "borrowCapGuardian == address(0)");
    ...
    function distributeSupplierComp(address cToken, address supplier, bool distributeAll) internal {
        ...
        compAccrued[supplier] = transferComp(supplier, supplierAccrued, distributeAll ? 0 : COMP_CLAIM_THRESHOLD);
    ...
    function distributeBorrowerComp(address cToken, address borrower, Exp memory marketBorrowIndex, bool distributeAll) internal {
        ...
        if (borrowerIndex.mantissa > 0) {
            ...
            compAccrued[borrower] = transferComp(borrower, borrowerAccrued, distributeAll ? 0 : COMP_CLAIM_THRESHOLD);
