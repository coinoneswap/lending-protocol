...
contract UniswapAnchoredView is UniswapConfig {
    ...
    /// @notice The chainlink price feeds
    ...
    /// @notice The event emitted when Chainlink price feeds changes
    ...
    bytes32 constant ROTATE_HASH = keccak256(abi.encodePacked("rotate"));
    ...
    constructor(OpenOraclePriceData priceData_,
                ...
                address[] memory _priceFeeds) UniswapConfig(configs) public {
        require(_priceFeeds.length == _tokens.length, "_priceFeeds.length != _tokens.length");
        for (uint256 i = 0; i < _tokens.length; i++) {
            chainlinkPriceFeeds[_tokens[i]] = AggregatorV3Interface(_priceFeeds[i]);
            emit ChainlinkPriceFeedUpdated(_tokens[i], _priceFeeds[i]);
        }
    ...
    function priceInternal(TokenConfig memory config) internal view returns (uint) {
        ...
        if (config.priceSource == PriceSource.FIXED_ETH) {
            uint usdPerEth = prices[ETH_HASH];
    ...
    function getUnderlyingPrice(address cToken) external view returns (uint) {
        ...
        return mul(1e18, getUnderlyingPriceInternal(config)) / config.baseUnit;
    }

    function getUnderlyingPriceInternal(TokenConfig memory config) internal view returns (uint) {
        AggregatorV3Interface priceFeed = chainlinkPriceFeeds[config.underlying];
    ...
    function invalidateReporter(bytes memory message, bytes memory signature) external {
        ...
        require(keccak256(abi.encodePacked(decodedMessage)) == ROTATE_HASH, "invalid message must be 'rotate'");
